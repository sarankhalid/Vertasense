...
export async function POST(request: NextRequest) {
    try {
        const body = await request.json();
        const { name, email, role, phone, organization_id, company_id, client_certification_id } = body;
        ...
        // Additional validation for company_id and client_certification_id when role is USER
        if (role === 'USER') {
            if (!company_id || !client_certification_id) {
                return NextResponse.json({ message: 'Company ID and Client Certification ID are required for USER role' }, { status: 400 });
            }
        }
        ...
        // Case 1: User exists (check if array has items)
        if (existingUser && existingUser.length > 0) {
            ...
            if (!existingUser[0].email_confirmed_at) {
                ...
                const { data: inviteData, error: inviteError } = await supabaseAdmin.auth.admin.inviteUserByEmail(
                    ...
                    {
                        redirectTo: `${url}/reset-password`,
                        data: { role, name, email, phone, organization_id, company_id, client_certification_id },
                    }
                );
            ...
            // Handle org_users table operations
            let orgUserData: any[] = [];
        ...
        const { data, error } = await supabaseAdmin.auth.admin.inviteUserByEmail(
            ...
            {
                redirectTo: `https://vertasense.vercel.app/reset-password`,
                data: { role, name, email, phone, organization_id, company_id, client_certification_id }, // Include company_id and client_certification_id for USER role
            }
        );
